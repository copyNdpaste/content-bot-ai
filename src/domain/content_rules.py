...
def required_landing_cta(account: str, lang: str) -> str:
...
def ensure_required_landing_cta(text: str, account: str, lang: str) -> str:
    """Normalize generated text so it ends with exactly one required CTA."""
    body = (text or "").rstrip()
    if not body:
        return body
    cta = required_landing_cta(account, lang)
    body = re.sub(
        r"\n*👉\s*(?:일본|한국|韓国|日本)[^\n]*tryproo\.com/?\s*$",
        "",
        body,
...
def enforce_payload_cta(payload: dict, account: str, lang: str) -> None:
    ...
    if not text.strip():
        return
    payload["text"] = ensure_required_landing_cta(text, account, lang)
