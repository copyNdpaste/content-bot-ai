# AI 이미지 자동 생성 - 외부 커맨드 연결

IG 게시 시 LLM (Claude Opus 4.7) 이 글 분위기에 맞는 영문 이미지 프롬프트를 생성하고,
content pipeline 이 설정된 외부 이미지 생성 커맨드를 호출한다. 이 커맨드는 Codex 기반 자동화든
별도 생성 스크립트든 상관없지만, 반드시 `prompt -> 로컬 이미지 파일` 계약을 지켜야 한다.
...
4. 결과 이미지는 `/tmp/codex-image-*.png` 로 저장된다.
...
```dotenv
CODEX_IMAGE_COMMAND=.venv/bin/python scripts/generate_gpt_image.py --prompt {prompt} --output {output} --width {width} --height {height}
...
`scripts/generate_gpt_image.py` 는 GPT Image 모델로 고품질 PNG 를 `{output}` 경로에 저장한다.
